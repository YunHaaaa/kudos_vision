...
#경기장의 hsv범위
field_minimum_condition = np.array([50, 30, 30])
field_maximum_condition = np.array([70, 250, 255])
addw
#경기장 컨투어 팽창 침식 강도 (경기장 공백공간의 노이즈 제거)
field_contour_dilate_p = 30
field_contour_erode_p = 10
...
class useful_function():
    ...
    def Image_mask(self, top_view_npArr, contour_img, mask_minimum_condition, mask_maximum_condition, roi_size, dilate_power, erode_power, gui_param_image):
        ...
        gray_top_view_npArr=cv2.cvtColor(top_view_npArr, cv2.COLOR_BGR2GRAY) # top_view_npArr을 gray로 변환.
        ...
        _, masked_hsv_top_view_npArr = cv2.threshold(gray_top_view_npArr, -1, 255,  cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    ...
    # 필드 팽창 및 침식강도인 field_contour_dilate_p, field_contour_erode_p 마지막으로 gui와 통신할때 전송하는 매개변수 리스트인 gui_param_image.
        ...
        contour_img = cv2.bitwise_and(top_view_npArr, top_view_npArr, mask = contour_img) 
        ...
        # 마지막으로 gui 하단의 마지막 화면을 만들기 위해 Otsu 알고리즘에 적용시킨다.
        # 이미지를 그레이 스케일로 읽기
        otsu_img = cv2.cvtColor(contour_img, cv2.COLOR_BGR2HSV)  
        #  OTSU 알고리즘 적용
        t, t_otsu = cv2.threshold(otsu_img, -1, 255,  cv2.THRESH_BINARY | cv2.THRESH_OTSU) 

        gui_param_image["otsu_field_img"] = t_otsu  # gui_param_image의 otsu_field_img에 t_otsu 저장.

        return t_otsu, gui_param_image
# gui 실행 함수.
...
def when_receive_yolo_image(ros_data, args):
    ...
    gui_param_image["otsu_field_img"]=0
