"#!/usr/bin/env python" # same c #include
IS_SIMULATOR = False 
SAVE_INPUT_VIDEO = False
import rospy
import roslib
from darknetb.msg import kudos_vision_ball_position as kvbp # msg form
...
# version sync
# 16.04 defauld = python 2.7 -> didnt apply module -> remove path
try:
    import sys
...
# IS_SIMULATOR = TRUE : SIMULATOR RUNNING => RUN UNDER CODE

# SIMULATOR = NO MORE USE => DO ####
...
send_preprocessed_img_jump_count = 10
...
class priROS():
    ...
    def yolo_talker(self, message_form):
        ...
        # blacked image msg parameter
    
    def img_talker(self, image_np):
        import cv2
        msg = CompressedImage() # provide module
        
        # compressed image variable setting
        msg.header.stamp = rospy.Time.now()
        msg.format = "jpeg" # image_np zip -> jpeg
        msg.data = np.array(cv2.imencode('.jpg', image_np)[1]).tostring() # store array form
        
        self.image_pub.publish(msg)
        # bounding box msg parameter (msg type form)

    def yolo_result_img_talker(self, image_np):
        import cv2
        print(np.shape(image_np)) # bounding box img -> 416, 416, 3 print
        msg = CompressedImage()
        msg.header.stamp = rospy.Time.now()
        msg.format = "jpeg"
...
class DataFormatTransfer():
    ...
    def get_one_center_from_detections(self, detections, label):
        max_confidence = 0 # threshold
        objectCenter = [default_x ,default_y] # bounding box center
        object_size = 0 # w, h
        for detections_index, detection in enumerate(detections):
            # detections > many parameter > label ( = obj name )
            #print(detection)#(label, confidence, (left, top, right, bottom))
            # detection = [label(ball, goal)], [0.9, 0.1](= confidence), [bounding box, x, y, w, h]
            if detection[0] == label:
                if float(detection[1])>max_confidence:
                    max_confidence = float(detection[1])
                    bbox = detection[2] 
                    
                    # bbox = bounding box information = [0][1][2][3] = x, y, w, h
    ...
    def get_mean_center_from_detections(self, detections, label):
        ...
        for detections_index, detection in enumerate(detections):
            #print(detection)#(label, confidence, (left, top, right, bottom))
            if detection[0] == label:
                bbox = detection[2] # x, y, w, h
                object_width_list.append(bbox[0]) # x x x x x x 
                object_height_list.append(bbox[1]) # y y y y y y 
                detect_flag = True
        if detect_flag == True:
            # mean -> center x, y
    ...
    # goalda -> line recognize disturb -> fill black
    # but in reality -> dificult, dosent effective, heavy, slow
    # so don't use
    def fill_black_on_detect_box(self, detections, img):
    	# initialize at up 73 line -> global -> bring that
        global mask_img
    ...
    def mapping_point_to_float_shape(self, npArr, objectCenter, objectSize):
        if objectCenter != [default_x, default_y]:
            ...
            objectSize = objectSize/((im_width_size+im_hight_size)/2) # 0~416 -> 0~1
...
# code option def => at kudos_darknet => parser function
    ...
    while True:
        ...
        # label / confidence score / bounding box / etc...
