...
# gui
# hue
# roi = intrested range - crop = region of interest
# pangchang = white range extend
# chimshik = black range extend
# variable range - use at dot
# gaussian filter - remove noise

# degree = bird eye view -> view change (raw -> vertical)
# field hsv => export only white/green range
# normal distribution
# dot export num limit
...
# pan tilt = vertical horizental degree
...
#  recognize after stadium area = white = error => prevent
...
# dot frame => standart range
# 대표 픽셀을 추출할 때 기준 거리 범위
# standard value
standard_pixel_max_distance = 60 # same max value
standard_pixel_min_distance = 5 # different min value
standard_pixel_distance_unit = 5
# real value
standard_pixel_distance = 0
# => filter real value -> standard value

# extract pixel num range set
standard_pixel_recommend_min_num = 22
standard_pixel_recommend_max_num = 23
# send montacarlo argorithm range set
standard_pixel_send_min_num = 15
standard_pixel_send_max_num = 30
# up two range qualified -> count start
...
# bird eye view opencv car - top view
#형 변환 사이즈 설정
# 2-3 = real img
# 0-2, 3-5 = black img [ 0 0 0 0 0 ...] 
# compress -> real img <- compress
empty_size_mul = 5
cp_size_left_loc = 2
cp_size_right_loc = 3

#로봇 고개 각도 설정
robot_desire_tilt = 60

#경기장의 hsv범위
field_minimum_condition = np.array([20, 230, 20])
field_maximum_condition = np.array([40, 250, 40])

#경기장 컨투어 팽창(white extend) 침식(black extend -> white range) 강도
...
start_point_x = 0
start_point_y = 0
start_point_orien = 0 # blue = present loc / orien = direction?
...
class useful_function():
    def __init__(self):
    	# pts = sadalicol ( sadalicol -> square )
        self.pts = np.zeros((4, 2), dtype=np.float32) # [0 0 0 0 0 ...]
        self.pts1 = 0
        self.pts2 = 0
